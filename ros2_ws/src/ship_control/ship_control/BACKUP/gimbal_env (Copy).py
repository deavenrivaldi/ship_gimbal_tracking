    ...
    def step(self, action):
        ...
        if not is_lost:
            if tracking_error_deg < 2.0:
                gear_ratio = 0.1   # 🎯 紅心區 (一檔：微調懸停)
            elif tracking_error_deg < 8.0:
                gear_ratio = 0.4   # 🟢 內圈區 (二檔：平滑靠近)
            elif tracking_error_deg < 20.0:
                gear_ratio = 0.8   # 🟡 邊緣區 (三檔：極速追趕)
            else:
                gear_ratio = 1.0   # 🟠 視野外邊緣 (四檔：油門全開拉回)
        ...
        if not is_lost:
            ...
            # 🌟 核心修正 2：加回防抖懲罰！
            # 嚴格禁止在鎖定目標時，搖桿忽左忽右！
            action_jitter_penalty = ((action_yaw_diff**2) + (action_pitch_diff**2)) * 2.0
            reward -= action_jitter_penalty
            ...
            panic_penalty = min(10.0, 2.0 + (time_since_vision * 2.0))
            ...
            if self.has_seen_target:
                ...
                if abs(pitch_pos) > 0.8:
                    # 超過 45 度後，越低頭懲罰越重，逼它想辦法把船留在中間
                    reward -= (abs(pitch_pos) - 0.8) * 2.0 + 5
            else:
                status_str = "🧭 開局全域盲搜中"
        ...
        # ========================================================
        target_time = 0.002  # 你可以隨時修改這個數字來換檔

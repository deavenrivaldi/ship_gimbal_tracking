...
class GimbalControllerNode(Node):
    ...
    def __init__(self):
        ...
        # Real-time gimbal world pose from Gazebo
    ...
    def shoot_projectile(self, position: Point, direction: Vector3, force: float):
        ...
        self.get_logger().info(
            f'Direction: ({direction.x:.2f}, {direction.y:.2f}, {direction.z:.2f}) '
            f'at yaw={math.degrees(self.get_parameter("gimbal_yaw").value):.1f}° '
            f'pitch={math.degrees(self.get_parameter("gimbal_pitch").value):.1f}°'
)
    ...
    def get_camera_world_pose(self):
        ...
        try:
            transform = self.tf_buffer.lookup_transform(
                'gimbal_world',                      # frame monde
                'gimbal_wam_v/gimbal/camera_link',   # frame caméra
                rclpy.time.Time()
            )
            return transform
    ...
    def quaternion_to_direction(self, q) -> Vector3:
        ...
        # Camera points in +X direction in its local frame
        # Apply quaternion rotation to [1, 0, 0]
    ...
    def test_loop(self):
        ...
        if yaw_target > self.JOINT_LIMITS['yaw_joint'][1] or yaw_target < self.JOINT_LIMITS['yaw_joint'][0]:
            ...
            if transform:
                ...
                spawn_pos = Point(
                    x=t.x + direction.x * offset,
                    y=t.y + direction.y * offset,
                    z=t.z + direction.z * offset
                )
                self.shoot_projectile(spawn_pos, direction, 15000.0)
            else:
                self.get_logger().warning('Camera pose not available, skipping shot')
